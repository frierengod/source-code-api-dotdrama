<?php
/**
 * Contoh Script Bridge API Dotdrama via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Dotdrama di sistem Bridge
function callDotdramaAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/dotdrama/api/v1";

// Contoh 1: Fitur Cari Film Dotdrama
echo "\n=====================[ 1. Pencarian Dotdrama ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callDotdramaAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callDotdramaAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callDotdramaAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/dotdrama ◀◀◀\n\n";
